We used Python 3.7.4 to run the code.
Download and Extract the zip file.
Create a data folder in the extracted directory and place the data files in the data folder. We will need the following five files inside the "data" folder:

traindata.txt
trainlabels.txt
testdata.txt
testlabels.txt
stoplist.txt

If you are on a Linux machine, you can do the following to run the code

chmod +x script.sh
./script.sh

The script.sh file uses the python3 command to run the main.py file.
If you have a GUI, then you can see the graphs opened in different windows. The program will also generate a png file and save it in in the directory from which the code is running.
So, if you are on a Linux server, you can transfer those png files to a workstation (Desktop/laptop) and watch them there.

The following python libraries are required.
pandas
numpy
sklearn
copy
matplotlib

If you do not have one of them, please install the library with pip3 and try to run the code again.